...
g1 = imnoise(g1,'gaussian',0,0.004);
g2 = imnoise(g2,'gaussian',0,0.004);
subplot(2,2,1), imshow(g1,[]);
subplot(2,2,2), imshow(g2,[]);
for a=1:x
    for b=1:y
        R1(a,b) = (G1(a,b)*abs(H(a,b))*abs(H(a,b)))/(H(a,b)*(abs(H(a,b))*abs(H(a,b))+0.0001));
    end
end

for a=1:x
    for b=1:y
        R2(a,b) = (G2(a,b)*abs(A(a,b))*abs(A(a,b)))/(A(a,b)*(abs(A(a,b))*abs(A(a,b))+0.0001));
    end
end
r1 = ifft2(R1);
r2 = ifft2(R2);
